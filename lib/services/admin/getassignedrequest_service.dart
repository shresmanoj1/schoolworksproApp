...
class AssignedRequestService extends ChangeNotifier {
  AdminRequestResponse? data;
  Future getassignedrequest(context, username) async {
  ...
  Future<AdminChangeTicketStatusResponse> updateStatus(
    ...
    try {
      return await http
          .put(Uri.parse(api_url2 + '/requests/update-status/$id'),
              headers: {
                'Authorization': 'Bearer $token',
                'Content-Type': 'application/json; charset=utf-8',
              },
  ...
  AcademicRequestResponse ? academic_request;
  Future getAcademicRequest(context, username) async {
    var client = http.Client();
    // var resultModel;
    final SharedPreferences sharedPreferences =
    await SharedPreferences.getInstance();
    ...
    var response = await client.get(
      Uri.parse('${api_url2}/requests/lecturer/academic-requests'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json; charset=utf-8',
      },
    ...
    academic_request = AcademicRequestResponse.fromJson(mJson);
    notifyListeners();
  }
}
