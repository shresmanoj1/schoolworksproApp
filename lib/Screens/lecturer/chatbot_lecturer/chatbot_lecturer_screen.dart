...
class ChatBotLecturer extends StatelessWidget {
  const ChatBotLecturer({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<ChatViewModel>(
      create: (_) => ChatViewModel(),
      child: const ChatBotBodyLecturer(),
    );
  }
}
...
class ChatBotBodyLecturer extends StatefulWidget {
  ...
  @override
  State<ChatBotBodyLecturer> createState() => _ChatBotLecturerState();
}

class _ChatBotLecturerState extends State<ChatBotBodyLecturer> {
  User? user;
  ...
  setD() async {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      _model.fetchChatLecturer(lecturerData).then((value) {
        setState(() {
          _allMessages.add(_model.lecturerMessages);
          typing = false;
        });
      });
    });
  ...
  tapMe(String message, String payload, ChatViewModel value) async {
    ...
    await value.fetchChatLecturer(lecturerData).then((e) {
      if (value.lecturerMessages.length > 1) {
        ...
        _scrollDown();
        addMultipleData(
            _allMessages.indexOf(_allMessages.last), value.lecturerMessages);
      } else {
        _scrollDown();
        setState(() {
          typing = true;
        });
        _allMessages.add(value.lecturerMessages);
        setState(() {
          typing = false;
        });
        _scrollDown();
